...
export default function ProductCard({product}: ProductCardProps) {
  const newTag = Date.now() - new Date(product.createdAt).getTime() < 1000*60*60*24*7;

    return <Link href={"/products" + product.id} className="card w-full bg-base-100 hover:shadow-xl transition-shadow">
        <figure>
